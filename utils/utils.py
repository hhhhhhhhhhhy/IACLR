...
from models.model import PairWiseModel
...
class BPRLoss:
    def __init__(self,
        ...
        self.lr = config['lr']
        self.intent_lr = config.get('intent_lr', self.lr * 0.05)
        
        intent_params = []
        other_params = []
        
        for name, param in recmodel.named_parameters():
            if 'intent' in name or 'attn' in name or 'scale' in name or 'projection' in name:
                intent_params.append(param)
            else:
                other_params.append(param)
                
        self.opt = optim.Adam([
            {'params': other_params, 'lr': self.lr},
            {'params': intent_params, 'lr': self.intent_lr}
        ])
